...
http.createServer((req, res) => {
    ...
    switch (req.url) {
        ...
        case '/':
            ...
            res.write("TITULO: Bienvenidos a DH Movies el mejor sitio para encontrar las mejores películas, incluso mucho mejor que Netflix, Cuevana y PopCorn.\n")
            res.write(`\n\n`)
            ...
            titulos.forEach(function(pelicula){
                res.write(pelicula)
                res.write(`\n\n`)
                })
            ...
            movies.forEach(function(pelireseña){
                res.write(`\n
    TITULO: ${pelireseña.title}\n
RESEÑA: ${pelireseña.overview}
                            `)
            })         
        ...
        case '/mas-votadas':
            let masvotadas = movies.filter(function(peliporpuntos){
            ...
            let votos = [];
            movies.forEach(function(puntaje){
                votos.push(puntaje.vote_average)
            })

            let promedio = votos.reduce(function(acumulador,voto){
                return  acumulador + voto
            })

            let totalPelis = movies.length

            let promedioFinal = promedio/totalPelis

            res.write(`*************          EL PROMEDIO DE VOTOS ES DE ${promedioFinal}         ***********\n\n`)

            res.write(`Las pelis mas votadas son: ${masvotadas.length}`)

            movies.forEach(function(listapelis){
                res.write(`\n
    TITULO: ${listapelis.title}\n
    PUNTAJE: ${listapelis.vote_average}\n
RESEÑA: ${listapelis.overview}
                            `)
            })         
        ...
        case '/sucursales':
            ...
            res.write(`Total de salas de cine: ${theaters.length}\n`)
            res.write(`LISTADO DE SALAS`)
            theaters.forEach(function(sala){
                res.write(`\n
    NOMBRE: ${sala.name}\n
...
DESCRIPCION : ${sala.description}
        ...
        res.write(`Titulo: Preguntas Frecuentas`)
        res.write( `Total de preguntas: ${faqs.length}`)
        
        faqs.forEach(function(preguntarespuesta){
            res.write(`\n
PREGUNTA: ${preguntarespuesta.faq_title}\n
RESPUESTA: ${preguntarespuesta.faq_answer}
                        `)
        })         
